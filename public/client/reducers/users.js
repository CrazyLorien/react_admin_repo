...
export default function users(state = { usersList: [], showReload: true }, action) {
    switch (action.type) {
        case 'RECEIVE_ALL_START':
            {
                return {...state, usersList: action.data, showReload: true };
            }
        case 'RECEIVE_ALL_SUCCESS':
            {
                let users = action.data.concat();
                return {...state, usersList: users, showReload: false };
            }
        ...
        case 'UPDATE_CLIENT_USER':
            {
                state.usersList = state.usersList.filter(x => x.id !== action.data.id);
                return {...state, usersList: state.usersList.concat(action.data), editedUser: action.data }
            }
